...
export default class MSDFText {
  ...
  async initialize(
    ...
    const targetLineHeight = isMultiLine ? 0.18 : 0.5;
    const lineHeightPx = fontData.common.lineHeight;
    let textScale = targetLineHeight / lineHeightPx;
    // For wide two-line text, match blog: max width ~2.9, centered, not huge
    const maxWidth = isMultiLine ? 2.9 : 4.2;
    if (textGeometry.layout.width * textScale > maxWidth) {
      textScale = maxWidth / textGeometry.layout.width;
    }
